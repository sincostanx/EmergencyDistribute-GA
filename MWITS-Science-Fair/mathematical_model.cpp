...
//consider change to double
long long g(int i,int j,int t,int d,int k){
    return min(cube(A(i,j,t+d,k)),cube(A(i,j,t+d,k))-cube(A(i,j,t+d,k)-k))/(d+1);
}
...
long long evaluate(){
    for(int i=1;i<=donation_center;i++){
        for(int j=1;j<=node[i];j++) value[i][j]+=sink[i][j].z+sink[i][j].r*40; //adjust time later
    }
    long long penalty=0;
    for(int i=1;i<=donation_center;i++){
...
int main(){
    ...
    for(int i=1;i<=donation_center;i++){
        for(int j=1;j<=node[i];j++){
            printf("%d %d: ",i,j);
            for(int k=0;k<ans[i][j].size();k++) printf("%d ",ans[i][j][k]);
            printf("\n");
        }
    }
    printf("P(40) = %I64d",evaluate());
}
