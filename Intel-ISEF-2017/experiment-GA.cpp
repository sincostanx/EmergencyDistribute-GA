...
int node,r[200],z[200],l[200],N[200],now;
    ...
    public: static void decrypt(){
        ...
        for(int k=0;k<solution.size();k++){
            if(solution[k]==-1) i++,j=0;
               ...
               // printf("Assign object %d %d to city %d\n",i,j,solution[k]);
                if(lock_down[k]==-1){
                    int next_state,minx=1e9;
                    for(int u=0;u<nodex;u++){
                        //if(Graph[u][pos[i][j]]) printf("there is connection from %d to %d\n",u,pos[i][j]);
                    ...
                    if(pos[i][j]<node&&pos[i][j]){
                        lock_down[k]=pos[i][j],time_lock_down[k]=now;
                    //    printf("object %d %d reach city %d\n",i,j,pos[i][j]);
                    }
                }
                //else printf("lock down object (%d,%d)\n",i,j);
                j++;
            }
        }
    }
...
void GA(){
    ...
    //printf("main system ok\n");
    printf("end genetic algorithm for day %d\n",now);
}
int main(){
    //freopen("hard_test_data.txt","r",stdin);
    //fp=fopen("hard_GA_output.txt","w");
    //FILE *np=fopen("hard_GA_raw_data.txt","w");
    while(scanf("%d",&T)&&T){
        memset(lock_down,-1,sizeof(lock_down));
        memset(Graph,0,sizeof(Graph));
        population.clear();
        input::graph_input(),now=1;
        for(;now<=T;now++){
        ...
        for(int k=0;k<solution.size();k++){
            if(solution[k]==-1) i++,j=0;
            else if(lock_down[k]!=-1) fprintf(np,"%d %d %d\n",i,j,lock_down[k]),j++;
        }
        fprintf(np,"-1\n");*/
    }
    fclose(fp),fclose(np);
